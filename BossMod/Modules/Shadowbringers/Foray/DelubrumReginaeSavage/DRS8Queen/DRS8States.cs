...
class DRS8States : StateMachineBuilder
{
    ...
    private void RelentlessPlay1(uint id, float delay)
    {
        ...
        // +3.0s: tethers/icons
        ActorCast(id + 0x10, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.ReversalOfForces, 3.1f, 4); // gunner casts automatic turret - starts 1s later, ends at the same time

        // +1.0s: tethers are replaced with statuses
        CastStart(id + 0x20, (uint)AID.NorthswainsGlow, 1.1f);
        // +0.1s: turrets spawn
        ActorCastStart(id + 0x21, Module.Enemies(OID.QueensGunner).FirstOrDefault, (uint)AID.Reading, 2.1f);
        ...
        ActorCastStart(id + 0x30, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.WindsOfWeight, 3.0f);
        ActorCastStart(id + 0x31, Module.Enemies(OID.QueensGunner).FirstOrDefault, (uint)AID.QueensShot, 0.2f)
    ...
    {
        Cast(id, (uint)AID.RelentlessPlay, delay, 5);
        ActorCast(id + 0x10, Module.Enemies(OID.QueensKnight).FirstOrDefault, (uint)AID.ShieldOmen, 3.2f, 3);

        ActorCastStart(id + 0x20, Module.Enemies(OID.QueensSoldier).FirstOrDefault, (uint)AID.DoubleGambit, 4.5f);
        Targetable(id + 0x21, false, 0.4f, "Disappear");
        ActorCastStart(id + 0x22, Module.Enemies(OID.QueensKnight).FirstOrDefault, (uint)AID.OptimalOffensive, 1.4f);
        ...
        ActorCastStart(id + 0x25, Module.Enemies(OID.QueensSoldier).FirstOrDefault, (uint)AID.SecretsRevealed, 3.2f); // right before cast start, 2 unsafe avatars are tethered to caster
    ...
    {
        Cast(id, (uint)AID.RelentlessPlay, delay, 5);
        ActorCastMulti(id + 0x10, Module.Enemies(OID.QueensKnight).FirstOrDefault, [(uint)AID.SwordOmen, (uint)AID.ShieldOmen], 3.1f, 3);

        // note: gunner starts automatic turret visual together with optimal play
        ActorCastMulti(id + 0x20, Module.Enemies(OID.QueensKnight).FirstOrDefault, [(uint)AID.OptimalPlaySword, (uint)AID.OptimalPlayShield], 6.5f, 5, false, "Cone + circle/donut")
        ...
        ActorCast(id + 0x30, Module.Enemies(OID.QueensGunner).FirstOrDefault, (uint)AID.TurretsTour, 1, 5, false, "Turrets start")
    ...
    {
        Cast(id, (uint)AID.RelentlessPlay, delay, 5);
        ActorCast(id + 0x10, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.Bombslinger, 3.1f, 3);
        // +0.9s: bombs spawn

        ActorCastStart(id + 0x20, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.ReversalOfForces, 3.2f); // icons/tethers appear ~0.1s before cast start
        ...
        ActorCastStartMulti(id + 0x31, Module.Enemies(OID.QueensSoldier).FirstOrDefault, [(uint)AID.FieryPortent, (uint)AID.IcyPortent], 2.2f)
        ...
        ActorCastStart(id + 0x33, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.AboveBoard, 0.5f);
    ...
    private void RelentlessPlay5(uint id, float delay)
    {
        ...
        ActorCastStart(id + 0x10, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.SoftEnrageW, 3.1f);
        ActorCastStart(id + 0x11, Module.Enemies(OID.QueensSoldier).FirstOrDefault, (uint)AID.SoftEnrageS, 3);
        ...
        ActorCastStart(id + 0x20, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.SoftEnrageW, 0.1f);
        ActorCastStart(id + 0x21, Module.Enemies(OID.QueensSoldier).FirstOrDefault, (uint)AID.SoftEnrageS, 3);
        ...
        ActorCastStart(id + 0x30, Module.Enemies(OID.QueensKnight).FirstOrDefault, (uint)AID.SoftEnrageK, 1.3f);
        ActorCastStart(id + 0x31, Module.Enemies(OID.QueensWarrior).FirstOrDefault, (uint)AID.SoftEnrageW, 3);
        ActorCastEnd(id + 0x32, Module.Enemies(OID.QueensKnight).FirstOrDefault, 2, false, "Raidwide 6")
           .SetHint(StateMachine.StateHint.Raidwide);
        ActorCastStart(id + 0x33, Module.Enemies(OID.QueensSoldier).FirstOrDefault, (uint)AID.SoftEnrageS, 1);
        ActorCastEnd(id + 0x34, Module.Enemies(OID.QueensWarrior).FirstOrDefault, 2, false, "Raidwide 7")
           .SetHint(StateMachine.StateHint.Raidwide);
        ActorCastStart(id + 0x35, Module.Enemies(OID.QueensGunner).FirstOrDefault, (uint)AID.SoftEnrageG, 1);
