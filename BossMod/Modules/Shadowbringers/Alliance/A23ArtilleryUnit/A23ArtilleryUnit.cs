...
class UpperLaser(BossModule module) : Components.GenericAOEs(module, ActionID.MakeSpell(AID.UpperLaser4))
{
    ...
    {
        if (_castersUpperLaser1.Count > 0)
            return _castersUpperLaser1.Select(c => new AOEInstance(_shapeUpperLaser1, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
        else if (_castersUpperLaser2.Count > 0)
            return _castersUpperLaser2.Select(c => new AOEInstance(_shapeUpperLaser2, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
        else
            return _castersUpperLaser3.Select(c => new AOEInstance(_shapeUpperLaser3, c.Position, c.CastInfo!.Rotation, c.CastInfo!.NPCFinishAt));
    }
