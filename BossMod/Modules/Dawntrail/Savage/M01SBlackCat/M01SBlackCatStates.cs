...
class M01SBlackCatStates : StateMachineBuilder
{
    ...
    {
        Cast(id, (uint)AID.BiscuitMaker, delay, 5, "Tankbuster 1")
    ...
    {
        Cast(id, (uint)AID.BloodyScratch, delay, 5, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    }

    private void QuadrupleCrossing(uint id, float delay)
    {
        Cast(id, (uint)AID.QuadrupleCrossingFirst, delay, 5)
        ...
        Cast(id + 0x10, (uint)AID.QuadrupleCrossingMid, 1.2f, 1);
        ...
        Cast(id + 0x20, (uint)AID.QuadrupleCrossingMid, 1.2f, 1);
        ComponentCondition<QuadrupleCrossingAOE>(id + 0x22, 0.6f, comp => comp.NumCasts > 0, "Cones 1");
        Cast(id + 0x30, (uint)AID.QuadrupleCrossingLast, 1.4f, 1);
    ...
    {
        Cast(id, (uint)AID.NineLives, delay, 3);
        CastMulti(id + 0x10, [(uint)AID.OneTwoPawBossRL, (uint)AID.OneTwoPawBossLR], 2, 5)
        ...
        Cast(id + 0x30, (uint)AID.Soulshade, 1.1f, 3);
        Cast(id + 0x40, (uint)AID.NineLives, 11.4f, 3);
        ComponentCondition<OneTwoPawShade>(id + 0x50, 3, comp => comp.NumCasts > 0, "Clone cleaves 1");
        CastStartMulti(id + 0x60, [(uint)AID.QuadrupleSwipeBoss, (uint)AID.DoubleSwipeBoss], 1);
        ...
        Cast(id + 0x70, (uint)AID.Soulshade, 2.1f, 3);

        CastMulti(id + 0x100, [(uint)AID.LeapingQuadrupleCrossingBossR, (uint)AID.LeapingQuadrupleCrossingBossL], 2.1f, 5)
            .ActivateOnEnter<QuadrupleCrossingProtean>();
        ComponentCondition<QuadrupleCrossingProtean>(id + 0x102, 1.8f, comp => comp.NumCasts > 0, "Jump + Proteans 1")
            .ActivateOnEnter<QuadrupleCrossingAOE>();
        Cast(id + 0x110, (uint)AID.LeapingQuadrupleCrossingBossMid, 1.2f, 1);
        ComponentCondition<QuadrupleCrossingProtean>(id + 0x112, 0.8f, comp => comp.NumCasts > 4, "Proteans 2")
            .DeactivateOnExit<QuadrupleCrossingProtean>();
        Cast(id + 0x120, (uint)AID.LeapingQuadrupleCrossingBossMid, 1.2f, 1)
        ...
        Cast(id + 0x130, (uint)AID.LeapingQuadrupleCrossingBossLast, 1.4f, 1);
    ...
    {
        Cast(id, (uint)AID.NineLives, delay, 3);
        CastMulti(id + 0x10, [(uint)AID.LeapingOneTwoPawBossLRL, (uint)AID.LeapingOneTwoPawBossLLR, (uint)AID.LeapingOneTwoPawBossRRL, (uint)AID.LeapingOneTwoPawBossRLR], 2.1f, 5)
        ...
        Cast(id + 0x30, (uint)AID.Soulshade, 2.2f, 3);
        Cast(id + 0x40, (uint)AID.NineLives, 2.1f, 3);

        CastMulti(id + 0x100, [(uint)AID.LeapingQuadrupleCrossingBossR, (uint)AID.LeapingQuadrupleCrossingBossL], 2.3f, 5)
            .ActivateOnEnter<QuadrupleCrossingProtean>();
        ComponentCondition<QuadrupleCrossingProtean>(id + 0x102, 1.8f, comp => comp.NumCasts > 0, "Jump + Proteans 1")
            .ActivateOnEnter<QuadrupleCrossingAOE>();
        Cast(id + 0x110, (uint)AID.LeapingQuadrupleCrossingBossMid, 1.2f, 1);
        ...
        ComponentCondition<QuadrupleCrossingAOE>(id + 0x132, 0.6f, comp => comp.NumCasts > 4, "Cones 2")
            .DeactivateOnExit<QuadrupleCrossingAOE>();
        Cast(id + 0x140, (uint)AID.Soulshade, 1.4f, 3);
    ...
    private void NineLives2Cleaves(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.TempestuousTear, 10.3f, 5);
    ...
    private void NineLives2Proteans(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.Nailchipper, 9.2f, 7)
    ...
    {
        Cast(id, (uint)AID.MouserVisual, delay, 10)
        ...
        Cast(id + 0x20, (uint)AID.Copycat, 7.6f, 3);
    }

    private void GrimalkinGale(uint id, float delay)
    {
        Cast(id, (uint)AID.GrimalkinGaleShockwave, delay, 6)
    ...
    private void Mouser2(uint id, float delay)
    {
        ...
        CastStartMulti(id + 0x100, [(uint)AID.Overshadow, (uint)AID.SplinteringNails], 8.4f)
            .ActivateOnEnter<ElevateAndEviscerate>()
            .ActivateOnEnter<ElevateAndEviscerateHint>()
            .ActivateOnEnter<ElevateAndEviscerateShockwave>()
        ...
        CastStartMulti(id + 0x110, [(uint)AID.Overshadow, (uint)AID.SplinteringNails], 7.4f);
        ComponentCondition<ElevateAndEviscerate>(id + 0x111, 4.7f, comp => comp.NumCasts >= 2, "Jump 2");
        CastEnd(id + 0x112, 0.3f);
        CastStartMulti(id + 0x120, [(uint)AID.Overshadow, (uint)AID.SplinteringNails], 7.4f);
        ComponentCondition<ElevateAndEviscerate>(id + 0x121, 4.7f, comp => comp.NumCasts >= 3, "Jump 3");
        CastEnd(id + 0x122, 0.3f);
        CastStartMulti(id + 0x130, [(uint)AID.Overshadow, (uint)AID.SplinteringNails], 7.4f);
    ...
    {
        Cast(id, (uint)AID.MouserEnrage, delay, 8)
            .ActivateOnEnter<Mouser>();
    ...
    {
        Cast(id, (uint)AID.RainingCatsFirst, delay, 6)
    ...
    {
        Cast(id, (uint)AID.Copycat, delay, 3);
        ...
        CastMulti(id + 0x30, [(uint)AID.OneTwoPawBossRL, (uint)AID.OneTwoPawBossLR], 0.2f, 5)
            .ActivateOnEnter<OneTwoPawBoss>();
