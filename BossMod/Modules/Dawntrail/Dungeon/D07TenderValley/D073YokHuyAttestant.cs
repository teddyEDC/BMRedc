...
class AncientWrath(BossModule module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.StatueActivate)
        {
            ...
            _aoes.Add(new AOEInstance(aoeShape, source.Position, source.Rotation, Module.WorldState.FutureTime(8.1f)));
        }
    }
...
{
    private static readonly List<Shape> union = [new Rectangle(new(-130, -475), 17.5f, 22)];
    private static readonly List<Shape> difference = [new Rectangle(new(-135.05f, -486.25f), 1.35f, 3.5f), new Rectangle(new(-124.4f, -471.4f), 1.3f, 3.75f)];
    private static readonly ArenaBounds arena = new ArenaBoundsComplex(union, difference);
    protected override void DrawEnemies(int pcSlot, Actor pc)
    {
        Arena.Actor(PrimaryActor);
        Arena.Actors(Enemies(OID.YokHuyAltar));
        Arena.Actors(Enemies(OID.YokHuyAltar2));
        Arena.Actors(Enemies(OID.YokHuyOrb));
    }
}
