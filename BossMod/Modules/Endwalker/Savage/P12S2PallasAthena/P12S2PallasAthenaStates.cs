...
class P12S2PallasAthenaStates : StateMachineBuilder
{
    ...
    private void SinglePhase(uint id)
    {
        ...
        Cast(id + 0xE0000, (uint)AID.Ignorabimus, 13.8f, 15, "Enrage");
    }

    private State Ultima(uint id, float delay)
    {
        return Cast(id, (uint)AID.UltimaNormal, delay, 5, "Raidwide")
    ...
    private void PalladianGraspResolve(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.PalladianGrasp2, 2.1f, 1);
    ...
    {
        Cast(id, (uint)AID.PalladianGrasp1, delay, 5)
    ...
    {
        Cast(id, (uint)AID.CrushHelm, delay, 5);
    ...
    {
        CastStart(id, (uint)AID.Ekpyrosis, delay);
        EkpyrosisResolve(id + 0x100, 4);
    }

    private void Gaiaochos1(uint id, float delay)
    {
        Cast(id, (uint)AID.Gaiaochos, delay, 7, "Raidwide + small arena 1 start")
            ...
            .SetHint(StateMachine.StateHint.Raidwide);
        Cast(id + 0x10, (uint)AID.SummonDarkness, 8.2f, 3);
        ...
        Cast(id + 0x100, (uint)AID.DemiParhelion, 6.0f, 3)
            .ActivateOnEnter<DemiParhelion>(); // note: casts start ~0.8s after boss cast ends
        CastMulti(id + 0x110, [(uint)AID.GeocentrismV, (uint)AID.GeocentrismC, (uint)AID.GeocentrismH], 3.2f, 7)
        ...
        Cast(id + 0x200, (uint)AID.UltimaGaiaochos, 1.6f, 5, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    ...
    {
        Cast(id, (uint)AID.Gaiaochos, delay, 7, "Raidwide + small arena 2 start")
            .ActivateOnEnter<Gaiaochos>()
            .DeactivateOnExit<Gaiaochos>()
            .OnExit(() => (Module.Arena.Center, Module.Arena.Bounds) = (new(100, 90), P12S2PallasAthena.SmallBounds))
            .SetHint(StateMachine.StateHint.Raidwide);
        Cast(id + 0x10, (uint)AID.SummonDarkness, 8.2f, 3);
        Cast(id + 0x20, (uint)AID.DemiParhelion, 3.1f, 3)
            .ActivateOnEnter<DemiParhelion>(); // note: casts start ~0.8s after boss cast ends
        CastStartMulti(id + 0x30, [(uint)AID.GeocentrismV, (uint)AID.GeocentrismC, (uint)AID.GeocentrismH], 3.2f);
        ...
        Cast(id + 0x100, (uint)AID.SummonDarkness, 5.1f, 3);
        CastStart(id + 0x110, (uint)AID.UltimaGaiaochos, 10.2f)
        ...
        CastStart(id + 0x200, (uint)AID.UltimaGaiaochos, 9.2f)
    ...
    {
        Cast(id, (uint)AID.ClassicalConcepts, delay, 7, "Raidwide + playstation 1 start")
        ...
        CastStart(id + 0x110, (uint)AID.PalladianRay, 1.6f)
    ...
    {
        Cast(id, (uint)AID.ClassicalConcepts, delay, 7, "Raidwide + playstation 2 start")
            ...
            .SetHint(StateMachine.StateHint.Raidwide);
        Cast(id + 0x10, (uint)AID.PantaRhei, 7.2f, 10);
        ...
        CastStart(id + 0x30, (uint)AID.PalladianRay, 1.6f)
    ...
    {
        CastStart(id, (uint)AID.CaloricTheory, delay)
    ...
    private void CaloricTheory2(uint id, float delay)
    {
        ...
        CastStart(id + 0x100, (uint)AID.Ekpyrosis, 26.2f)
    ...
    {
        Cast(id, (uint)AID.Pangenesis, delay, 7, "Raidwide + towers start")
            .ActivateOnEnter<Pangenesis>() // statuses appear 0.7s after cast end
            .SetHint(StateMachine.StateHint.Raidwide);
        Cast(id + 0x10, (uint)AID.Pantheos, 4.1f, 4);
        ...
        CastStart(id + 0x1000, (uint)AID.PalladianGrasp1, 10.4f)
