...
class P12S1AthenaStates : StateMachineBuilder
{
    ...
    {
        Cast(id, (uint)AID.Paradeigma, delay, 3)
    ...
    {
        Cast(id, (uint)AID.Paradeigma, delay, 3);
        Cast(id + 0x10, (uint)AID.EngravementOfSouls, 3.7f, 4);
    ...
    {
        Cast(id, (uint)AID.SuperchainTheory1, delay, 5)
            .ActivateOnEnter<SuperchainTheory1>();
        Cast(id + 0x10, (uint)AID.EngravementOfSouls, 3.2f, 4);
    ...
    private void EngravementOfSouls3(uint id, float delay)
    {
        ...
        Cast(id + 0x20, (uint)AID.UnnaturalEnchainment, 3.2f, 7, "Fixed towers")
    ...
    private void Palladion(uint id, float delay)
    {
        ...
        Cast(id + 0x100, (uint)AID.Palladion, 2.4f, 8, "Limit cut start")
        ...
        Cast(id + 0x400, (uint)AID.TheosUltima, 1.8f, 7, "Raidwide")
    ...
    {
        Cast(id, (uint)AID.SuperchainTheory2A, delay, 5)
    ...
    {
        Cast(id, (uint)AID.SuperchainTheory2B, delay, 5)
            .ActivateOnEnter<SuperchainTheory2B>(); // first tethers appear 1.5s after cast end
        Cast(id + 0x10, (uint)AID.Paradeigma, 3.2f, 3);
        CastStart(id + 0x20, (uint)AID.Parthenos, 5.2f);
        ...
        ComponentCondition<RayOfLight>(id + 0x60, 2.4f, comp => comp.NumCasts > 0, "Lines")
            .DeactivateOnExit<RayOfLight>();
        CastStart(id + 0x70, (uint)AID.UnnaturalEnchainment, 2.4f);
