...
class C033StaticeStates : StateMachineBuilder
{
    ...
    private void SinglePhase(uint id)
    {
        ...
        Cast(id + 0xB0000, (uint)AID.Enrage, 3.2f, 10, "Enrage");
    }

    private void Aero(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.SAero : (uint)AID.NAero, delay, 5, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    }

    private void ShockingAbandon(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.SShockingAbandon : (uint)AID.NShockingAbandon, delay, 5, "Tankbuster")
            .SetHint(StateMachine.StateHint.Tankbuster);
    }

    private void TrickReload1(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.STrickReload : (uint)AID.NTrickReload, delay, 4)
            .ActivateOnEnter<TrickReload>();
        Cast(id + 0x10, _savage ? (uint)AID.STrapshooting1 : (uint)AID.NTrapshooting1, 10.7f, 4)
            .ActivateOnEnter<Trapshooting>();
        ComponentCondition<Trapshooting>(id + 0x12, 4.1f, comp => comp.NumResolves > 0, "Stack/spread");
        Cast(id + 0x20, _savage ? (uint)AID.STriggerHappy : (uint)AID.NTriggerHappy, 2.1f, 4.3f)
        ...
        Cast(id + 0x30, _savage ? (uint)AID.SRingARingOExplosions : (uint)AID.NRingARingOExplosions, 4.2f, 3)
            .ActivateOnEnter<RingARingOExplosions>();
        Cast(id + 0x40, _savage ? (uint)AID.STrapshooting2 : (uint)AID.NTrapshooting2, 10.2f, 4);
    ...
    private void TrickReload2(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.STrickReload : (uint)AID.NTrickReload, delay, 4)
            .ActivateOnEnter<TrickReload>();
        Cast(id + 0x10, _savage ? (uint)AID.SRingARingOExplosions : (uint)AID.NRingARingOExplosions, 10.7f, 3)
            .ActivateOnEnter<RingARingOExplosions>();
        Cast(id + 0x20, _savage ? (uint)AID.SDartboardOfDancingExplosives : (uint)AID.NDartboardOfDancingExplosives, 2.2f, 3)
            .ActivateOnEnter<Dartboard>(); // bullseye statuses are applied on 3 players ~0.6s after cast end
        Cast(id + 0x30, _savage ? (uint)AID.STrapshooting2 : (uint)AID.NTrapshooting2, 9.7f, 4)
            .ActivateOnEnter<Trapshooting>();
        ...
        Cast(id + 0x100, _savage ? (uint)AID.SSurpriseBalloon : (uint)AID.NSurpriseBalloon, 8.5f, 4);
        Cast(id + 0x110, _savage ? (uint)AID.SBeguilingGlitter : (uint)AID.NBeguilingGlitter, 3.2f, 4);
        CastStart(id + 0x120, _savage ? (uint)AID.STriggerHappy : (uint)AID.NTriggerHappy, 3.2f);
        ...
        Cast(id + 0x130, _savage ? (uint)AID.STrapshooting2 : (uint)AID.NTrapshooting2, 0.7f, 4)
    ...
    private void Intermission(uint id, float delay)
    {
        ...
        Cast(id + 0x10, _savage ? (uint)AID.SRingARingOExplosions : (uint)AID.NRingARingOExplosions, 1.5f, 3)
            .ActivateOnEnter<RingARingOExplosions>();
        Cast(id + 0x20, _savage ? (uint)AID.SPresentBox : (uint)AID.NPresentBox, 2.1f, 3)
        ...
        Cast(id + 0x30, _savage ? (uint)AID.SFireworks : (uint)AID.NFireworks, 2.1f, 3)
    ...
    {
        Cast(id, _savage ? (uint)AID.SPinwheelingDartboard : (uint)AID.NPinwheelingDartboard, delay, 3)
            .ActivateOnEnter<Dartboard>()
            .ActivateOnEnter<FireSpread>() // first cast starts ~2.6s after cast-end
            .ActivateOnEnter<Fireworks>()
            .ActivateOnEnter<Fireworks2Hints>();
        Cast(id + 0x10, _savage ? (uint)AID.SFireworks : (uint)AID.NFireworks, 5.7f, 3);
    ...
    {
        Cast(id, _savage ? (uint)AID.SBeguilingGlitter : (uint)AID.NBeguilingGlitter, delay, 4);
        Cast(id + 0x10, _savage ? (uint)AID.STrickReload : (uint)AID.NTrickReload, 3.2f, 4)
            .ActivateOnEnter<TrickReload>();
        Cast(id + 0x20, _savage ? (uint)AID.STrapshooting1 : (uint)AID.NTrapshooting1, 10.7f, 4)
            .ActivateOnEnter<Trapshooting>();
        ComponentCondition<Trapshooting>(id + 0x22, 4.1f, comp => comp.NumResolves > 0, "Stack/spread");

        Cast(id + 0x30, _savage ? (uint)AID.SPresentBox : (uint)AID.NPresentBox, 0.1f, 3);
        // +1.0s: spawn 4x staffs
        Cast(id + 0x40, _savage ? (uint)AID.SRingARingOExplosions : (uint)AID.NRingARingOExplosions, 3.6f, 3)
            .ActivateOnEnter<BeguilingGlitter>()
            .ActivateOnEnter<NFaerieRing>(!_savage)
            .ActivateOnEnter<SFaerieRing>(_savage);
        Cast(id + 0x50, _savage ? (uint)AID.STriggerHappy : (uint)AID.NTriggerHappy, 3.1f, 4.3f)
        ...
        CastStart(id + 0x60, _savage ? (uint)AID.STrapshooting2 : (uint)AID.NTrapshooting2, 2.1f)
            .ActivateOnEnter<RingARingOExplosions>();
        CastEnd(id + 0x61, 4);
