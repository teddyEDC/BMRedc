...
class C013ShadowcasterStates : StateMachineBuilder
{
    ...
    private void SinglePhase(uint id)
    {
        ...
        Cast(id + 0xC0000, (uint)AID.Enrage, 4.8f, 10, "Enrage");
    }

    private void ShowOfStrength(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.SShowOfStrength : (uint)AID.NShowOfStrength, delay, 5, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    }

    private void FiresteelFracture(uint id, float delay)
    {
        Cast(id, _savage ? (uint)AID.SFiresteelFracture : (uint)AID.NFiresteelFracture, delay, 5)
    ...
    {
        Cast(id, (uint)AID.InfernBrand, delay, 4);
        Cast(id + 0x10, (uint)AID.CrypticPortal, 3.2f, 4)
            .ActivateOnEnter<NPortalsBurn>(!_savage)
            .ActivateOnEnter<SPortalsBurn>(_savage); // eobjanims ~0.8s after cast end
        Cast(id + 0x20, (uint)AID.FiresteelStrike, 6.2f, 4.9f)
        ...
        Cast(id + 0x40, (uint)AID.BlessedBeacon, 3.1f, 4.9f);
    ...
    private void InfernBrand2(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.CrypticFlames, 3.2f, 8.3f)
        ...
        CastStart(id + 0x20, (uint)AID.CastShadow, 6.7f);
        ...
        CastStart(id + 0x30, _savage ? (uint)AID.SFiresteelFracture : (uint)AID.NFiresteelFracture, 7.7f);
    ...
    private void InfernBrand3(uint id, float delay)
    {
        ...
        Cast(id + 0x10, (uint)AID.InfernWave, 4.2f, 4);
        Cast(id + 0x20, (uint)AID.Banishment, 4.5f, 4)
        ...
        Cast(id + 0x30, (uint)AID.InfernWard, 4.2f, 4);
    ...
    private void InfernBrand5(uint id, float delay)
    {
        ...
        Cast(id + 0x20, (uint)AID.CrypticFlames, 4.2f, 8.3f)
            .ActivateOnEnter<CrypticFlames>(); // note: statuses appear right before cast start
        ...
        CastStart(id + 0x40, (uint)AID.PureFire, 8);
        ...
        CastStart(id + 0x60, (uint)AID.CastShadow, 4.9f);
