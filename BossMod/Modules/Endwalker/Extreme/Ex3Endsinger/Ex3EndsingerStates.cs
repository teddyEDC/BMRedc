...
class Ex3EndsingerStates : StateMachineBuilder
{
    ...
    {
        return Cast(id, (uint)AID.Telos, delay, 5, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    }

    private void Hubris(uint id, float delay)
    {
        CastStart(id, (uint)AID.Hubris, delay);
        CastEnd(id + 1, 5)
    ...
    {
        return CastMulti(id, [(uint)AID.ElenchosCenter, (uint)AID.ElenchosSides], delay, 6, "Elenchos")
    ...
    {
        Cast(id, (uint)AID.Telomania, delay, 5);
    ...
    {
        Cast(id, (uint)AID.ElegeiaUnforgotten, delay, 5);
    ...
    {
        Cast(id, (uint)AID.Fatalism, delay, 3, "Double Planets")
    ...
    {
        Cast(id, (uint)AID.Fatalism, delay, 3, "Quad Planets")
            .ActivateOnEnter<Planets>();
        Cast(id + 0x1000, (uint)AID.Fatalism, 7.2f, 3)
            .SetHint(StateMachine.StateHint.PositioningStart);
        ...
        {
            // TODO: components
            Cast(id + 0x2000, (uint)AID.Katasterismoi, 11.2f, 3, "Towers");
    ...
    {
        // TODO: components...
        Cast(id, (uint)AID.Katasterismoi, delay, 3, "Towers")
            .DeactivateOnExit<Planets>();
        Cast(id + 0x1000, (uint)AID.GripOfDespair, 3.2f, 5, "Tethers");
    ...
    {
        CastStart(id, (uint)AID.Eironeia, delay)
            .DeactivateOnExit<Planets>();
        CastEnd(id + 1, 5)
    ...
    {
        CastStart(id, (uint)AID.AporrhoiaUnforgotten, delay)
            .SetHint(StateMachine.StateHint.PositioningStart);
    ...
    {
        Cast(id, (uint)AID.TwinsongAporrhoia, delay, 3, "Twinsong");
        ...
        Cast(id + 0x400, (uint)AID.TheologicalFatalism, 5.2f, 3, "Rewind")
            .SetHint(StateMachine.StateHint.PositioningStart);
    ...
    {
        Cast(id, (uint)AID.DespairUnforgotten, delay, 3, "Despair 1")
            .ActivateOnEnter<DespairUnforgotten>();
        Cast(id + 0x100, (uint)AID.DespairUnforgotten, 10.2f, 3, "Despair 2");
        Elenchos(id + 0x200, 6.2f);
        Cast(id + 0x300, (uint)AID.DespairUnforgotten, 5.2f, 3, "Despair 3");
        Cast(id + 0x400, (uint)AID.TheologicalFatalism, 11.2f, 3, "Rewind");
    ...
    {
        Cast(id, (uint)AID.EndsongAporrhoia, delay, 3, "Endsong");
        Cast(id + 0x100, (uint)AID.Endsong, 5.2f, 3)
    ...
    {
        Cast(id, (uint)AID.Enrage, 9.3f, 5);
