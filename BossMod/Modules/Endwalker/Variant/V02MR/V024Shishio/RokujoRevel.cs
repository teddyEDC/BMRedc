...
class RokujoRevel(BossModule module) : Components.GenericAOEs(module)
{
    ...
    private readonly List<Actor> _clouds = [.. module.Enemies(OID.Raiun), .. module.Enemies(OID.Rairin)];
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch ((AID)spell.Action.ID)
        {
            case AID.SmokeaterFirst:
            case AID.SmokeaterRest:
                ++_numBreaths;
                break;
            case AID.SmokeaterAbsorb:
