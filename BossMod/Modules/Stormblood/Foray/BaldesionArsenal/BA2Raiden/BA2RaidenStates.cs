...
class BA2RaidenStates : StateMachineBuilder
{
    ...
    {
        Cast(id, (uint)AID.Shingan, delay, 4, "Tankbuster")
            .SetHint(StateMachine.StateHint.Tankbuster);
    }

    private void SpiritsOfTheFallen(uint id, float delay)
    {
        Cast(id, (uint)AID.SpiritsOfTheFallen, delay, 4, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    }

    private void Thundercall(uint id, float delay)
    {
        Cast(id, (uint)AID.Thundercall, delay, 3, "Arena wall");
    }
    ...
    private void BallLightningLateralZantetsuken(uint id, float delay)
    {
        ...
        CastStartMulti(id + 0x10, [(uint)AID.LateralZantetsuken1, (uint)AID.LateralZantetsuken2], 0.8f, "Half room cleave starts");
    ...
    private void BallLightningLateralZantetsukenUltimateZantetsuken(uint id, float delay)
    {
        ...
        CastStartMulti(id + 0x50, [(uint)AID.LateralZantetsuken1, (uint)AID.LateralZantetsuken2], 0.1f, "Half room cleave starts");
        ...
        CastStart(id + 0xC0, (uint)AID.UltimateZantetsuken, 2.1f, "Enrage start");
    ...
    private void UltimateZantetsuken(uint id, float delay)
    {
        ...
        CastStart(id + 0x50, (uint)AID.UltimateZantetsuken, 1.6f, "Enrage start");
    ...
    {
        Cast(id, (uint)AID.BoomingLament, delay, 4, "Circle AOE");
    ...
    {
        ComponentCondition<CloudToGround>(id, delay, comp => comp.Lines.Count != 0, "Exaflares start");
        Cast(id + 0x10, (uint)AID.LevinwhorlVisual, 6.3f, 8, "Raidwide")
            .SetHint(StateMachine.StateHint.Raidwide);
    ...
    private void WhirlingZantetsukenSilentLevin(uint id, float delay)
    {
        ...
        Cast(id + 0x50, (uint)AID.BoomingLament, 1.1f, 4, "Circle AOE")
            .ResetComp<SilentLevin>();
    }
}
