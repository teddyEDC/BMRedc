...
class BA1ArtStates : StateMachineBuilder
{
    ...
    {
        Cast(id, (uint)AID.Thricecull, delay, 4, "Tankbuster")
    ...
    private void LegendCarverSpinner(uint id)
    {
        ...
        CastMulti(id + 0x10, [(uint)AID.Legendcarver, (uint)AID.Legendspinner], 4, 4.5f, "Inverse of previous AOE");
    }

    private void AcallamNaSenorach(uint id, float delay)
    {
        Cast(id, (uint)AID.AcallamNaSenorach, delay, 4, "Raidwide")
    ...
    private void Mythcall(uint id, float delay)
    {
        ...
        CastMulti(id + 0x10, [(uint)AID.Legendcarver, (uint)AID.Legendspinner], 6, 4.5f, "In/Out AOE");
    ...
    {
        Cast(id, (uint)AID.Mythcall, delay, 2, "Spawn spears");
        Cast(id + 0x10, (uint)AID.PiercingDarkVisual, 6.1f, 2.5f, "Spreads");
        CastMulti(id + 0x20, [(uint)AID.Legendcarver, (uint)AID.Legendspinner], 1.6f, 4.5f, "In/Out AOE");
    ...
    {
        Cast(id, (uint)AID.LegendaryGeas, delay, 4, "Circle AOE + stop moving");
    ...
    private void GloryUnearthedPitfall(uint id, float delay)
    {
        ...
        CastStart(id + 0x10, (uint)AID.Pitfall, 2.5f, "Proximity AOE start");
